...
document.addEventListener("DOMContentLoaded",
    async () => {
        ...
        async function connect(event) {
            ...
            const response = await fetch(`/api/v2/init/${MAP_ID}`, {credentials: "include"});
            
            if (!response.ok) {
                alert(`Error connecting to map: ${response.status} ${response.statusText}`);
                return;
            }
            ...
            current_ni = data.ni;
            current_nj = data.nj;
            postCooldown = data.timeout / 1000000000;
            lastPostTime = Date.now();

            draw_map(current_ni, current_nj, data.data);
        ...
        function draw_map(ni, nj, data) {
            ...
            for (let i = 0; i < ni; i++) {
                for (let j = 0; j < nj; j++) {
                    ...
                    pixel.dataset.i = i;
                    pixel.dataset.j = j;
                    pixel.addEventListener("click", () => set_pixel(i, j));
                    pixel.addEventListener("mouseover", () => {
                        document.getElementById("coord-display").textContent = `Pixel: (${i}, ${j})`;
                    });
        ...
        async function refresh() {
            const response = await fetch(`/api/v2/get/${MAP_ID}`, {credentials: "include"});
            if (!response.ok) {
        ...
        async function set_pixel(i, j) {
            ...
            if (elapsed < postCooldown) {
                alert(`Cooldown: Wait ${(postCooldown - elapsed).toFixed(1)}s`);
                return;
            }
            
            const [r, g, b] = getPickedColorInRGB();
            const response = await fetch(`/api/v2/set/${MAP_ID}`, {
                method: 'POST',
                headers: {
                    'API-KEY': API_KEY,
        ...
        setInterval(() => {
            const now = Date.now();
            const elapsed = (now - lastPostTime) / 1000;
            const remaining = Math.max(0, postCooldown - elapsed);
            document.getElementById("cooldown-display").textContent = 
                remaining > 0 ? `Cooldown: ${remaining.toFixed(1)}s` : "Ready";
        }, 100);

        //TODO: for advanced users: it could be useful to be able to
        // choose the color from a pixel?
        // (Already implemented via right-click on pixel)
        ...
        // Create info display divs
        const coordDisplay = document.createElement("div");
        coordDisplay.id = "coord-display";
        coordDisplay.style.cssText = "position: fixed; top: 10px; right: 10px; font-size: 12px; background: rgba(0,0,0,0.7); color: white; padding: 5px 10px; border-radius: 3px;";
        document.body.appendChild(coordDisplay);
        
        const cooldownDisplay = document.createElement("div");
        cooldownDisplay.id = "cooldown-display";
        cooldownDisplay.style.cssText = "position: fixed; top: 40px; right: 10px; font-size: 12px; background: rgba(0,0,0,0.7); color: white; padding: 5px 10px; border-radius: 3px;";
